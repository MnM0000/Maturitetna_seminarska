...
G = 6.673 * (10**(-11))
SCALE = 10**9 # skala
nSCALE = SCALE # pomozna skala za izracun povecave
...
class planet():
    ...
    def update(self):
        ...
        self.x += self.x_vel
        self.y += self.y_vel
        pg.draw.circle(screen, self.colour, (self.x, self.y), self.rad if self.rad > 2 else 2) # if stavek zaradi prikaza planetov (razdalje so v vesolju velike in se planetov ne bi videlo, če jih ne bi povečal)

    def pos(self):
        return self.x, self.y # vrne svoje koordinate

def distance(item1, item2):
    # racun razdalje in lastnosti vektorja
    x1, y1 = item1.pos()
    try:
        x2, y2 = item2.pos()
    except:
    ...
    length = vect*SCALE
    dist = [length, (x1-x2)/vect, (y1-y2)/vect] # razdalja v metrih, normirana vrednost v x in y smeri
    return dist

def grav_effect(pl1, pl2, r):
    # racunanje premikov in preverjanje trkov
    m1 = pl1.mass
    m2 = pl2.mass
    v1 = G*m2*SPD/(r[0]**2) # izracun hitrosti 1. predmeta
    v2 = G*m1*SPD/(r[0]**2) # izracun hitrosti 2. predmeta
    s1 = [v1*r[1]*SPD/SCALE, v1*r[2]*SPD/SCALE] # izracun poti in mnozenje z normirano vrednostjo vektorja v x in y smeri zato, da se dobi premik v x in y smeri
    s2 = [v2*r[1]*SPD/SCALE, v2*r[2]*SPD/SCALE]
    if r[0]/SCALE <= (pl1.rad + pl2.rad): # preverjanje za trk
        planets.remove(pl1)
        planets.remove(pl2)
        pl1.x_vel -= s1[0]
        pl1.y_vel -= s1[1]
        ...
        if pl1.mass > pl2.mass:
            vx = (m1*pl1.x_vel + m2*pl2.x_vel)/(m1+m2) # izracun hitrosti novega -zdruzenega- planeta s formulo za gibalno kolicino
        ...
        # dodajanje vrednosti poti na "hitrost" predmeta -posodabljamo zaslon nekajkrat na sekundo, zato je to dejansko pot, ki jo prištejemo vsako iteracijo, kar pomeni v bistvu hitrost
        pl1.x_vel -= s1[0]
        pl1.y_vel -= s1[1]
        pl2.x_vel += s2[0]
        pl2.y_vel += s2[1]
...
def rad_draw(pl1, czoom):
    # izris polmera in razdalje med planetoma
...
if mode == 1:
    # primer 1 -> dokaz pravilnega delovanja simulacije gravitacije
    Earth = planet(5.97*mass_mult, width/2, height/2 + 147.1, 30.29*10**3*SPD/SCALE, 0, blue, 1/5)
    planets = [Sun, Earth]
elif mode == 0:
    # primer 0 -> primer simulacije celega osoncja (nenatancno zaradi slabih podatkov, a vseeno dovolj nazorno)
    planets = [Sun]

    # dodajanje vseh planetov glede na vhodne podatke iz spletne strani Nase v seznam planetov, ki se posodabljajo in prikazujejo na zaslonu
    ...
    # primer 2 -> dokaz delovanja preverjanja trkov (pri nizkih hitrostih)
...
t0 = time.time()
while True:
    clock.tick(FPS) # dodeljevanje osvezitev na sekundo
    for event in pg.event.get(): # preverjanje za dogodek
        if event.type == pg.QUIT: sys.exit()
        elif event.type == pg.MOUSEWHEEL:
            # povecevanje/pomanjsevanje
            if event.y < 1:
                xzoom = zoom
                nSCALE = SCALE + yzoom
                for p in planets:
    ...
    # klic funkcije grav_effect - vsak element v seznamu planets z vsakim
    if len(planets) != 1:
        for i in range(len(planets)-1):
            for ii in range(i+1,len(planets)):
                x = grav_effect(planets[i], planets[ii], distance(planets[i], planets[ii]))
                if x == 1: # ce pride do trka pojdi ven iz zanke
                    break   
    
    screen.fill(black)
    
    # klic funkcije rad_draw, ce je nacin enak 1
    if mode == 1:
        for x in planets:
            if x.x != width/2 or x.y != height/2:
                rad_draw(x, nSCALE)

    # posodobitev polozaja planetov in njihov izris na 'platno'
    for p in planets:
        p.update()
    
    # zapis casa v dnevih/10 dnevih zgoraj desno na 'platnu'
    img2 = text.render(str((time.time() - t0).__round__(2)), 1, white)
    screen.blit(img2, (10, 10))

    # posodabljanje zaslona glede na 'platno'
    pg.display.update()
